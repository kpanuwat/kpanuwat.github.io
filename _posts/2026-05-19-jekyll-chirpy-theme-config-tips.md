...
| `render_with_liquid` | `false` | ปิด Liquid processing (ป้องกัน {% raw %}`{{ }}`{% endraw %} ใน code block ถูก parse) |
...
ป้องกัน Chirpy parse {% raw %}`{{ variable }}`{% endraw %} ใน code block เป็น Liquid template (ต้องการ Jekyll 4.0+)
...
- `render_with_liquid: false` ปิด Liquid **ทั้ง post** รวมถึง {% raw %}`{% include %}`{% endraw %} tags ด้วย — ใช้เฉพาะเมื่อจำเป็น
